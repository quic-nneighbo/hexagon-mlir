// RUN: linalg-hexagon-opt %s -pass-pipeline='builtin.module(func.func(extend-pack-frontier{parallels-only=false}))' -split-input-file | FileCheck %s --check-prefixes=CHECK
//===------------------------------------------------------------------===//
// lit tests adapted from upstream that cover different aspects of layout propagation
// but not incorporating some of the hexagon-mlir requests for our own in-house
// test development such as: 1) having a core heavy linalg op such as matmul
// around which to expand linalg.pack and linalg.unpack frontier. Also 2)
// these tests do not document some of the limitations/pass assumptions
// that are good to keep track of in hexagon-mlir (e.g. linalg.pack padding
// support in layout propagation)
//===------------------------------------------------------------------===//
...
// CHECK-DAG: #[[$MAP0:.+]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2, d3, d4, d5)>
// CHECK-DAG: #[[$MAP1:.+]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, 0, 0, 0, d4, d5)>
// CHECK-DAG: #[[$MAP2:.+]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (0, d1, 0, 0, d4, d5)>
// CHECK-DAG: #[[$MAP3:.+]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d2, d1, d3, d4, d5)>
...
// CHECK-SAME:      inner_dims_pos = [3, 1] inner_tiles = [16, 32]
// CHECK-SAME:    into %[[ARG0_EMPTY]]
// CHECK:         %[[ARG1_EMPTY:.+]] = tensor.empty() : tensor<100x1x1x1x16x32xi32>
// CHECK:         %[[PACKED_ARG1:.+]] = linalg.pack %[[ARG1]]
// CHECK-SAME:      inner_dims_pos = [3, 1] inner_tiles = [16, 32]
// CHECK-SAME:    into %[[ARG1_EMPTY]]
// CHECK:         %[[ARG2_EMPTY:.+]] = tensor.empty() : tensor<1x4x1x1x16x32xi32>
// CHECK:         %[[PACKED_ARG2:.+]] = linalg.pack %[[ARG2]]
// CHECK-SAME:      inner_dims_pos = [3, 1] inner_tiles = [16, 32]
// CHECK-SAME:    into %[[ARG2_EMPTY]]
// CHECK:         %[[RES:.+]] = linalg.generic
// CHECK-SAME:      indexing_maps = [#[[$MAP0]], #[[$MAP1]], #[[$MAP2]], #[[$MAP3]]]
// CHECK-SAME:      ins(%[[PACKED_ARG0]], %[[PACKED_ARG1]], %[[PACKED_ARG2]]
// CHECK-SAME:      outs(%[[DEST]]
...
//This test covers the *WOULD_BREAK_DOMINANCE* case in ExtendPackUnpackFrontier.cpp
#map0 = affine_map<(d0, d1) -> (d0, d1)>
